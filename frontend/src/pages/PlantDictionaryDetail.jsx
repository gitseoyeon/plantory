...
const PlantDictionaryDetail = () => {
  ...
  useEffect(() => {
    const fetchPlantDetail = async () => {
      try {
        ...
        console.log("🌱 상세조회 데이터:", response.data); // ✅ 백엔드 응답 로그
  ...
  const handleBack = () => {
    if (location.state?.fromSearch && query) {
      navigate(`/dictionary?query=${encodeURIComponent(query)}`, { replace: true });
    } else {
      navigate("/dictionary");
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-md overflow-hidden">
        ...
        <div className="p-6">
          ...
          <div className="text-gray-700 space-y-2">
            <p><strong>다른 이름:</strong> {plant.otherName || "정보 없음"}</p>
            <p><strong>원산지:</strong> {plant.origin || "정보 없음"}</p>
            <p><strong>물 주기:</strong> {plant.water || "정보 없음"}</p>
            <p><strong>햇빛:</strong> {plant.sunlight || "정보 없음"}</p>
            <p><strong>토양:</strong> {plant.soil || "정보 없음"}</p>
            <p><strong>설명:</strong> {plant.description || "등록된 설명이 없습니다."}</p>
          </div>
        </div>
      </div>
    </div>
